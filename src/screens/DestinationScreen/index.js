...
import {formatter, setAccessToken} from '../../utils';
import {api} from "../../api";
...
function ZakazScreen(){
    ...
    const [itemss, setItemss] = useState([
        {label: 'Пешком', value: 'peshkom'},
        {label: 'Lgkov', value: 'Lgkov'},
        {label: 'Gruz', value: 'gruz'},
    ]);
    ...
    const {origin, distance, dtae, cena, phone, cennost, comments, names, namesp} = location
    ...
    const distanceMOWBKK = getDistanceFromLatLonInKm(
        location.origin.latitude, location.origin.longitude, location.distance.latitude, location.distance.longitude
    )

    const onCor = (data, details) => {
    ...
    const onCord = (data, details) => {
        ...
        setLocation(prevState =>({...prevState, cena: distanceMOWBKK}))
    }
    console.log(cena)
    ...
    const onChange = ( event, selectedDate)=>{
        ...
        let fD = temp.getDate() +'/'+(temp.getMonth())+'/'+(temp.getFullYear())
    ...
    const submitSignInForm = () =>{
        api
            .getZakaz({origin: origin, distance: distance, names: names, phone:phone, dtae: dtae,comments: comments, cena:cena, namesp:namesp, cennost:cennost,  strahovka:valuesStrahovka, ves: value, type: values })
            .then(() => {})
            .catch(()=> {});
    }
    return (
       <SafeAreaView style={{flex: 1}}>
           ...
           >
                      ...
                      <TouchableOpacity>
                          <View style = {styles.card}>
                              <View style ={styles.view2}>
                                  <Svg
                                      style ={[styles.image2, {alignItems: "flex-start"}]} fill="#335616"
                                      focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="TimerIcon"
                                      tabIndex="-1" title="Timer">
                                  ...
                                  <Text style ={styles.title}>{t("как можно скорее")}</Text>
                                  <Text style ={[styles.title, {fontSize: 30, fontWeight: '800'}]}>
                                      {t("От 149 Р")}
                                      <Svg
                                          style={[styles.image2, {width: 20, height: 20}]}
                                          focusable="false" aria-hidden="true" viewBox="0 0 24 24"
                                          data-testid="PlayCircleOutlineIcon" tabIndex="-1" title="PlayCircleOutline">
                           ...
                           <View style = {styles.card}>
                               <View style ={styles.view2}>
                                   <Svg
                                       style ={[styles.image2, {alignItems: "flex-start"}]} fill="#335616"
                                       focusable="false" aria-hidden="true" viewBox="0 0 24 24"
                                       data-testid="CalendarMonthIcon" tabIndex="-1" title="CalendarMonth">
                       ...
                       <TouchableOpacity>
                           <View style = {styles.card}>
                               <View style ={styles.view2}>
                                   <Svg
                                       style ={[styles.image2, {justifyContent: "space-evenly", flexDirection: "row"}]} fill="#335616"
                                       focusable="false" aria-hidden="true" viewBox="0 0 24 24"
                                       data-testid="CalendarMonthIcon" tabIndex="-1" title="CalendarMonth">
                                       <Path d="M19 4h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V10h14v10zM9 14H7v-2h2v2zm4 0h-2v-2h2v2zm4 0h-2v-2h2v2zm-8 4H7v-2h2v2zm4 0h-2v-2h2v2zm4 0h-2v-2h2v2z"/>
                                       <Text style={{position: "absolute", marginLeft: 40, paddingHorizontal: 10, paddingVertical: 5, color: '#fff', backgroundColor: "#2f2e2e",borderRadius: 50}}>Для бизнеса</Text>
                                   </Svg>
                                   <Text style ={styles.title}>{t("В интервал 4 часа")}</Text>
                                   <Text style ={[styles.title, {fontSize: 30, fontWeight: '800'}]}>
                                       {t("От 149 Р")}
                                       <Text style={{color: "#8a6b6b", fontSize: 10, fontWeight: '500'}}>
                                           {t('этот адрес')}
                                       </Text>
                                       <Svg
                                           style={[styles.image2, {width: 20, height: 20}]}
               ...
               <Text style={[styles.title, {textAlign: "center"}]}>{t('Курьер будет на адресах в удобное для вас время')}</Text>
                   ...
                   <>
                       <View style={{ marginTop: 50}}>
                           <GooglePlacesAutocomplete
                               ...
                               styles = {autoComplete}
                               debounce ={400}
                               onPress={(data, details)=>{onCor(data, details)}}
                           />
                           <GooglePlacesAutocomplete
                               ref={ref}
                               placeholder={`${t('Адрес')}`}
                               ...
                               styles = {autoComplete}
                               debounce ={400}
                               onPress={(data, details)=>{onCord(data, details)}}
                           />
                           ...
                           <View style={{ marginTop: 20}}>
                               ...
                               <View style={[styles.in, {flexDirection: "row", justifyContent: "space-evenly"}]}>
                                   <TouchableOpacity style={[styles.TextInput, {alignItems: "center", justifyContent: "center"}]} onPress={()=> shownMode('date')}>
                                       <Text style={{color: colors.placeholder, fontSize: 20, fontWeight: "bold"}}>{text}</Text>
                                   </TouchableOpacity>
                                   <TouchableOpacity style={[styles.TextInput, {alignItems: "center", justifyContent: "center"}]}  onPress={()=> shownMode('time')}>
                                       <Text style={{color: colors.placeholder, fontSize: 20, fontWeight: "bold"}}>{time}</Text>
                                   </TouchableOpacity>
                                   {isDatePickerVisible && (
                                       <DateTimePicker
                               ...
                               <View style={styles.in}>
                                   <View style={{flexDirection: "row", alignItems: "center", justifyContent: "space-evenly"}}>
                                       <Svg fill="#335616" width={30} height={30}
                                            focusable="false" aria-hidden="true" viewBox="0 0 24 24"
                                            data-testid="DirectionsWalkIcon" tabIndex="-1" title="DirectionsWalk">
                                       ...
                                       <Text style={{ color: colors.placeholder, fontSize: 27, fontWeight: "bold"}}>{t('Поручение для курьера')}</Text>
                                   </View>
                               ...
                               <View style={[styles.in, {paddingHorizontal: 15}]}>
                                   <DropDownPicker
               ...
               <View style={{flexDirection: "row", justifyContent: "space-between", alignItems: "center"}}>
                   ...
                   <Button title={t('отправить')} onPress={submitSignInForm}  buttonStyle={[styles.TextInput, {backgroundColor: 'rgb(70,85,31)', paddingHorizontal: 30}]}/>
               </View>
               <View style={{marginTop: 20}}/>
           </ScrollView>
       </SafeAreaView>
